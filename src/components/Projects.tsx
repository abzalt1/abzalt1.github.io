...
export default function Projects({ onOpenLightbox }: { onOpenLightbox: (images: string[], index: number) => void }) {
    const projects = [
        ...
        },
        {
            ...
            result: "Запущен эстетичный и быстрый сайт-портфолио с собственной независимой админ-панелью. Благодаря связке Next.js и Cloudflare R2 удалось добиться мгновенной загрузки тяжелых фотографий и видео по всему миру, а клиент полностью избавился от ежемесячных оплат за подписки на конструкторы сайтов.",
            tasks: [
                "Проектирование минималистичного интерфейса с акцентом на контент и чистую типографику.",
                "Внедрение плавных анимаций и нестандартного скролла для улучшения визуального опыта без ущерба производительности.",
                "Оптимизация доставки медиафайлов: интеграция Cloudflare R2 для быстрого хостинга тяжелого контента с нулевой стоимостью трафика.",
                "Разработка кастомной CMS: удобное управление портфолио и загрузка новых съемок прямо со смартфона.",
                "Детальная адаптация под мобильные устройства и оптимизация изображений для Retina-дисплеев."
            ],
